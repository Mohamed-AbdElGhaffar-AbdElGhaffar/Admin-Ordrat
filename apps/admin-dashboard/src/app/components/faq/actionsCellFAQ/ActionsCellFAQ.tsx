...
const ActionsCellFAQ: React.FC<ActionsCellProps> = ({ row, lang, view = false }) => {
    
  const { openModal } = useModal();
  const { setUpdateFaq } = useFileContext();

  const handleOpenModal = () => {
    openModal({
      view: <UpdateFAQForm lang={lang} row={row.original} onSuccess={()=>setUpdateFaq(true)}/>,
      customSize: '700px',
    });
  };
