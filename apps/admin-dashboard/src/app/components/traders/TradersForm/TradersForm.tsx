...
import styles from './TradersForm.module.css';
...
export default function TradersForm({
  ...
  const text = {
    ...
    submit: lang === 'ar' ? 'إضافة تاجر' : 'Add Trader',
  ...
  const mainFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        console.error(error);
        toast.error(lang === 'ar' ? 'حدث خطأ أثناء الإضافة' : 'An error occurred while adding the seller');
      }
    },
  });
