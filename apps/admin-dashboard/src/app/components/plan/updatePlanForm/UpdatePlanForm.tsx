...
import styles from './UpdatePlanForm.module.css';
...
export default function UpdatePlanForm({
    ...
    try {
      ...
      if (responseAr.ok && responseEn.ok) {
        ...
        const features = dataAr.planFeatures.map((featureAr: any) => {
          const correspondingEn = dataEn.planFeatures.find(
            (featureEn: any) => featureEn.feature.id === featureAr.feature.id
          );

          return {
            id: featureAr.feature.id,
            nameAr: featureAr.feature.name,
            nameEn: correspondingEn?.feature.name || '',
            descriptionAr: featureAr.feature.description,
            descriptionEn: correspondingEn?.feature.description || '',
          };
        });
        ...
        toast.error(lang === 'ar' ? 'خطأ في جلب البيانات' : 'Error fetching plan data');
      }
    } catch (error) {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (response.ok) {
          ...
          toast.error(lang === 'ar' ? 'حدث خطأ أثناء التعديل' : 'Failed to update the plan');
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!loading) {
      fetchAllFeatures();
      const isDataChanged =
        JSON.stringify(mainFormik.values) !== JSON.stringify(initialValues) ||
        JSON.stringify(selectedFeatures) !== JSON.stringify(initialFeatures);

      setIsChanged(isDataChanged);
    }
  }, [mainFormik.values, selectedFeatures, initialValues, initialFeatures, loading]);
